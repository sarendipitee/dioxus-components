...
test("exposes trigger, listbox, groups, and option state through ARIA", async ({
  page,
}) => {
  ...
  await expect(option(page, /^🍊 Orange$/i)).toHaveAttribute(
    "aria-disabled",
    "true",
  );
});

test("controlled selection updates value and calls callback exactly once", async ({
  page,
}) => {
  const root = fruitRoot(page);
  const trigger = fruitTrigger(page);
  await expect(page.getByTestId("fruit-select-value")).toHaveText(/Apple/i);
  await expect(page.getByTestId("fruit-select-callback-count")).toHaveText(
    "Callbacks: 0",
  );
  await trigger.click();
...
test("keyboard navigation skips disabled options and supports Home, End, and typeahead", async ({
  page,
}) => {
  const trigger = fruitTrigger(page);
  await trigger.click();
...
test("Escape restores trigger focus and outside click closes the menu", async ({
  page,
}) => {
  const trigger = fruitTrigger(page);
  await trigger.click();
...
test("disabled group propagates aria-disabled and cannot be selected", async ({
  page,
}) => {
  ...
  await trigger.click();
  const group = listbox(page).getByRole("group", {
    name: "Unavailable fruits",
  });
...
test("disabled root trigger cannot open", async ({ page }) => {
  const trigger = page
    .getByTestId("disabled-root-select-root")
    .first()
    .locator("#disabled-root-select")
    .first();
...
test("focused options have a visual cue", async ({ page }) => {
  ...
  await expect(option(page, /Apple/i)).toBeFocused();
  await expect
    .poll(() => visualStyle(option(page, /Apple/i)))
    .not.toEqual(before);
});
...
test("multi-select retains its interaction coverage", async ({ page }) => {
  ...
  const trigger = page
    .getByRole("button")
    .filter({ hasText: /Pepperoni|Mushroom|Onion/ });
  await trigger.click();
  const menu = listbox(page);
