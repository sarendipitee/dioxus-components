...
test("single accordion exposes state and switches the visible panel", async ({ page }) => {
  ...
  await expect(accordion.getByText("Update your profile and account preferences.")).toBeVisible();
  ...
  await expect(account).toHaveAttribute("aria-expanded", "false");
  await expect(accordion.getByText("Update your profile and account preferences.")).toBeHidden();
  await expect(accordion.getByText("Review invoices and payment methods.")).toBeVisible();

  await billing.click();
  await expect(billing).toHaveAttribute("aria-expanded", "false");
  await expect(accordion.getByText("Review invoices and payment methods.")).toBeHidden();
});

test("multiple accordion keeps panels open and honors non-collapsible default", async ({ page }) => {
  ...
  await expect(accordion.getByText("Orders are shipped within two business days.")).toBeVisible();
  ...
  await expect(shipping).toHaveAttribute("aria-expanded", "true");
  await expect(accordion.getByText("Returns are accepted within thirty days.")).toBeVisible();
});

test("disabled items and disabled accordions cannot be activated", async ({ page }) => {
  const accordion = await loadAccordion(page, "single-accordion");
  ...
  await expect(accordion.getByText("Archived project settings are unavailable.")).toBeHidden();

  const disabledAccordion = page.locator("#disabled-accordion");
  const disabledTrigger = disabledAccordion.getByRole("button", { name: "Disabled accordion" });
  await expect(disabledTrigger).toBeDisabled();
  await expect(items(disabledAccordion).first()).toHaveAttribute("data-disabled", "true");
});

test("keyboard activation and vertical focus navigation skip disabled items", async ({ page }) => {
  const accordion = await loadAccordion(page, "single-accordion");
  const account = accordion.getByRole("button", { name: "Account settings" });
  const billing = accordion.getByRole("button", { name: "Billing" });
  const notifications = accordion.getByRole("button", { name: "Notifications" });
