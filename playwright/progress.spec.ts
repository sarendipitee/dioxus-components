...
function progressbar(page: Page) {
  return page
    .getByRole("progressbar", { name: "Progressbar Demo", exact: true })
    .first();
}

test("exposes determinate semantics, text, and global attributes", async ({
  page,
}) => {
...
test("switches to indeterminate semantics", async ({ page }) => {
  ...
  const progress = progressbar(page);
  await page
    .getByRole("button", { name: "Set indeterminate", exact: true })
    .first()
    .click();
...
test("clamps values to the supported range", async ({ page }) => {
  ...
  const progress = progressbar(page);
  await page
    .getByRole("button", { name: "Set above max", exact: true })
    .first()
    .click();
  ...
  await page
    .getByRole("button", { name: "Set below min", exact: true })
    .first()
    .click();
...
test("uses zero progress when max is not positive", async ({ page }) => {
  ...
  const progress = progressbar(page);
  await page
    .getByRole("button", { name: "Set max zero", exact: true })
    .first()
    .click();
  ...
  await page
    .getByRole("button", { name: "Set max negative", exact: true })
    .first()
    .click();
  await expect(progress).toHaveAttribute("aria-valuemax", "0");
  await expect(progress).toHaveAttribute("aria-valuenow", "0");
