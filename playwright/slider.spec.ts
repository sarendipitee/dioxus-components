...
test('test', async ({ page }) => {
  await page.goto('/components/slider', { timeout: 20 * 60 * 1000 }); // Increase timeout to 20 minutes
...
test('drag survives pointercancel (iPad system gesture)', async ({ page }) => {
  ...
  await page.goto('/components/slider', { timeout: 20 * 60 * 1000 });
...
test('dynamic min/max', async ({ page }) => {
  await page.goto('/components/slider/block#dynamic_range', { timeout: 20 * 60 * 1000 });
...
test('range two thumbs', async ({ page }) => {
  await page.goto('/components/slider/block#range', { timeout: 20 * 60 * 1000 });
