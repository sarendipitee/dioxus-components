...
test.describe("Tabs", () => {
  test("exposes tab roles, reciprocal ARIA links, selection, and automatic keyboard behavior", async ({ page }) => {
    ...
    for (let index = 0; index < 3; index += 1) {
      ...
      const panel = page.locator(`[role="tabpanel"]#${await tab.getAttribute("aria-controls")}`);
    ...
    await expect(page.locator(`[role="tabpanel"]#${await tabs.nth(1).getAttribute("aria-controls")}`)).toBeVisible();
  });

  test("manual activation separates focus from selection and skips disabled tabs", async ({ page }) => {
  ...
  test("vertical tabs use vertical arrows and ignore horizontal arrows", async ({ page }) => {
  ...
  test("controlled tabs count genuine changes, support deactivation, and retain inactive panels", async ({ page }) => {
    ...
    await expect(page.getByTestId("tabs-controlled-list")).toHaveAttribute("role", "tablist");
    await expect(page.getByTestId("tabs-controlled-overview-trigger")).toHaveAttribute("role", "tab");
    await expect(page.getByTestId("tabs-controlled-overview-panel")).toHaveAttribute("role", "tabpanel");
    const output = page.getByTestId("tabs-controlled-output");
    await expect(root).toHaveAttribute("aria-label", "Controlled tabs fixture");
    await expect(output).toHaveText("Selected value: overview; Callback count: 0");
    const metrics = root.getByRole("tab", { name: "Metrics" });
    await metrics.click();
    await expect(output).toHaveText("Selected value: metrics; Callback count: 1");
    await metrics.click();
    await expect(output).toHaveText("Selected value: none; Callback count: 2");
    await metrics.click();
    await expect(output).toHaveText("Selected value: metrics; Callback count: 3");
    await expect(page.getByTestId("tabs-controlled-overview-panel")).toBeHidden();
    await expect(root.locator('[role="tabpanel"]').first()).toBeAttached();
    await expect(root.locator('[role="tabpanel"]').first()).toHaveAttribute("data-state", "inactive");
  });

  test("controlled tabs add, select, and remove Reports dynamically", async ({ page }) => {
    await page.goto("/components/tabs/block#controlled");
    const root = page.getByTestId("tabs-controlled-root");
    ...
    await reports.click();
    await expect(page.getByTestId("tabs-controlled-output")).toContainText("Selected value: reports");
    await expect(root.getByRole("tabpanel", { name: "Reports" })).toBeVisible();
    await toggle.click();
    await expect(reports).toHaveCount(0);
    await expect(page.getByTestId("tabs-controlled-output")).toContainText("Selected value: overview");
  });

  test("non-looping tabs stop roving focus at each boundary", async ({ page }) => {
