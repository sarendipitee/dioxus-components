...
const demoUrl = (demo: string) => `/components/combobox/block#${demo}`;
...
test("exposes listbox ARIA state and linkage across open and close", async ({
  page,
}) => {
  ...
  await expect
    .poll(() =>
      page.evaluate((id) => {
        ...
        return (
          linked?.getAttribute("role") === "listbox" && linked.checkVisibility()
        );
      }, controls),
    )
    .toBe(true);
...
test("Escape dismisses without changing the committed value or focus", async ({
  page,
}) => {
  await page.goto(URL, { timeout: 30 * 1000 });
  await page.waitForLoadState("domcontentloaded");
...
test("opens from the focused input with the keyboard", async ({ page }) => {
  ...
  await expect(trigger).toHaveAttribute("aria-expanded", "true");
  await expect(
    list(page).getByRole("option", { name: "Next.js" }),
...
test("ArrowUp opens at the last option with a visual active item", async ({
  page,
}) => {
  await page.goto(URL, { timeout: 30 * 1000 });
  await page.waitForLoadState("domcontentloaded");
...
test("keeps filtered options during keyboard close animation", async ({
  page,
}) => {
  await page.goto(URL, { timeout: 30 * 1000 });
  await page.waitForLoadState("domcontentloaded");
  ...
  if (await closingContent.count()) {
    ...
    await expect(
      closingList.getByRole("option", { name: "SvelteKit" }),
    ).toBeVisible();
...
test("disabled options are exposed but skipped by keyboard selection", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("combobox", { name: "Disabled combobox" }),
  ).toBeDisabled();
...
test("dynamic option mutation keeps list open, while an ordinary outside click dismisses it", async ({
  ...
  await expect(
    list(page).getByRole("option", { name: "SvelteKit" }),
  ).toBeVisible();
  await expect(
    list(page).getByRole("option", { name: "SolidStart" }),
  ).toBeVisible();
  await page.keyboard.press("ArrowDown");
  await expect(
    list(page).getByRole("option", { name: "Next.js" }),
...
test("delayed option arrival replaces empty state and supports keyboard selection", async ({
  page,
}) => {
  await page.goto(demoUrl("dynamic"), { timeout: 30 * 1000 });
  await page.waitForLoadState("domcontentloaded");
...
test("virtualized demo shows visible options when opened", async ({ page }) => {
  ...
  const trigger = page.getByRole("combobox", {
    name: "Virtualized option picker",
  });
  await trigger.focus();
  await expect(trigger).toBeFocused();
  await page.keyboard.press("ArrowDown");
  ...
  await expect(
    menu.getByRole("option", { name: "Option 0", exact: true }),
  ).toBeVisible();
  await expect(
    menu.getByRole("option", { name: "Option 1", exact: true }),
  ).toBeVisible();
...
test("virtualized ArrowUp opens at final logical option", async ({ page }) => {
  ...
  const last = list(page).getByRole("option", {
    name: "Option 999",
    exact: true,
  });
...
test("virtualized demo keeps scrollHeight stable while scrolling", async ({
  page,
}) => {
  await page.goto(demoUrl("virtualized"), { timeout: 30 * 1000 });
  await page.waitForLoadState("domcontentloaded");
...
test("touch selection commits and closes", async ({ browser, browserName }) => {
  test.skip(
    browserName === "firefox",
    "Firefox does not support mobile contexts",
  );

  const { defaultBrowserType: _defaultBrowserType, ...iphone } =
    devices["iPhone 12"];
