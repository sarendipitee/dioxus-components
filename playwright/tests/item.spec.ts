...
async function loadItemDemo(page: Page, demo: string) {
  await page.goto(`${ITEM_BASE_URL}#${demo}`, {
    timeout: 30 * 1000,
    waitUntil: "domcontentloaded",
  });
}

test("main Item exposes noninteractive default structure and disabled action", async ({
  page,
}) => {
  await loadItemDemo(page, "main");
  const basic = page.locator(
    '#dx-preview-block-root [data-testid="basic-item"]',
  );
  ...
  await expect(basic.locator('[data-slot="item-title"]')).toHaveJSProperty(
    "tagName",
    "DIV",
  );
  await expect(basic.locator('[data-slot="item-title"]')).toHaveText(
    "Basic Item",
  );
  await expect(
    basic.locator('[data-slot="item-description"]'),
...
test("verified Item is a native focused anchor with icon media and Enter activation", async ({
  page,
}) => {
  await loadItemDemo(page, "main");
  ...
  await expect(link.locator('[data-slot="item-media"]')).toHaveAttribute(
    "data-variant",
    "icon",
  );
...
test("variant demo renders default, outline, and muted Items with content and actions", async ({
  page,
}) => {
  ...
  ).resolves.toEqual(["default", "outline", "muted"]);
  for (const [index, title] of [
    ...
    await expect(
      items.nth(index).locator('[data-slot="item-title"]'),
    ).toHaveText(title);
  }
  await expect(items.locator('[data-slot="item-description"]')).toHaveCount(3);
  await expect(
    items.getByRole("button", { name: "Open", exact: true }),
  ).toHaveCount(3);
});

test("size demo exposes default and small Item sizing with media variants", async ({
  page,
}) => {
  ...
  await expect(
    items.nth(1).locator('[data-slot="item-media"]'),
  ).toHaveAttribute("data-variant", "default");
});

test("image demo renders three linked image Items with accessible metadata", async ({
  page,
}) => {
  ...
  await expect(items).toHaveCount(3);
  await expect(
    items.evaluateAll((nodes) => nodes.every((node) => node.tagName === "A")),
  ).resolves.toBe(true);
  for (const [index, alt] of [
    ...
    await expect(
      items.nth(index).locator('[data-slot="item-media"] img'),
...
test("group demo preserves list semantics, decorative separators, avatars, and keyboard actions", async ({
  page,
}) => {
  ...
  for (let index = 0; index < 2; index += 1) {
    ...
    await expect(separators.nth(index)).toHaveAttribute(
      "data-orientation",
      "horizontal",
    );
  }
