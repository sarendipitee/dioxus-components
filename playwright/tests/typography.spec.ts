...
test("typography preserves semantic heading hierarchy independently of visual variants", async ({
  page,
}) => {
  ...
  await expect(
    preview.getByRole("heading", { level: 2, name: "Shared typography" }),
  ).toBeVisible();
  await expect(
    preview.getByRole("heading", {
      level: 3,
      name: "Semantic h3 with medium visual size",
    }),
  ).toBeVisible();

  const fixture = preview.locator("#typography-semantic-fixture");
  await expect(
    fixture.getByRole("heading", {
      level: 1,
      name: "Typography level one heading",
    }),
  ).toHaveText("Heading level one");
  await expect(
    fixture.getByRole("heading", { level: 3, name: "Heading level three" }),
  ).toBeVisible();
  await expect(
    fixture.getByRole("heading", { level: 6, name: "Heading level six" }),
  ).toBeVisible();
});

test("text selects supported paragraph, span, div, and label elements", async ({
  page,
}) => {
  await gotoTypographyDemo(page);
  ...
  const preview = page.locator("#dx-preview-block-root");
  await expect(
    preview.locator("p", { hasText: "Use Text and Heading" }),
  ).toBeVisible();
  await expect(
    preview.locator("span", { hasText: "Inline accent span" }),
  ).toBeVisible();

  const fixture = preview.locator("#typography-semantic-fixture");
  await expect(
    fixture.locator('div[data-typography-text-element="div"]'),
  ).toHaveText(
...
test("typography forwards global attributes while retaining supplied classes", async ({
  page,
}) => {
  await gotoTypographyDemo(page);

  const fixture = page.locator(
    "#dx-preview-block-root #typography-semantic-fixture",
  );
  ...
  await expect(division).toHaveAttribute(
    "aria-label",
    "Typography semantic division",
  );
});

test("typography exposes truncation and line-clamp state on the rendered elements", async ({
  page,
}) => {
  await gotoTypographyDemo(page);
  ...
  const truncated = fixture.locator(
    'p[data-typography-text-element="paragraph"]',
  );
