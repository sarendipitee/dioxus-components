...
test("exposes semantic horizontal separator and forwarded attributes", async ({ page }) => {
  ...
  await expect(page.locator("#separator-status")).toHaveAttribute("aria-live", "polite");
  await expect(page.locator("#separator-status")).toHaveText(
    "orientation: horizontal; decorative: false",
  );
});

test("reactively updates vertical orientation and decorative semantics", async ({ page }) => {
  await gotoSeparatorDemo(page);
  const fixture = separator(page);

  await page.getByRole("button", { name: "Toggle orientation", exact: true }).click();
  ...
  await page.getByRole("button", { name: "Toggle decorative", exact: true }).click();
