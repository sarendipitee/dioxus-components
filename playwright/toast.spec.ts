...
test("each typed toast renders its leading variant icon", async ({ page }) => {
  ...
  for (const { button, type } of TYPED_TOASTS) {
    ...
    const toast = page.locator(
      `[role="alertdialog"][data-type="${type}"]:visible`,
    );
...
test("loading toast shows the spinner instead of a leading icon", async ({
  page,
}) => {
  ...
  await page
    .locator("#dx-preview-block-root")
    .getByRole("button", { name: "Save (success)" })
    .click();

  const toast = page.locator(
    '[role="alertdialog"][data-type="loading"]:visible',
  );
  await expect(toast).toBeVisible();
...
test("toasts can be opened and dismissed individually", async ({ page }) => {
  ...
  const trigger = page
    .locator("#dx-preview-block-root")
    .getByRole("button", { name: "Show permanent" });
  await trigger.click();
  await trigger.click();
...
test("collapsed stack keeps variable-height toasts at measured edge offsets", async ({
  page,
}) => {
  await page.goto("/components/toast/block#main");
...
test("expanded stack collapses without replaying entry animation", async ({
  page,
}) => {
  await page.goto("/components/toast/block#permanent");
...
test("description toasts expose exact content and linked accessible names", async ({
  page,
}) => {
  ...
  const toast = page
    ...
    .filter({ hasText: "Event has been created" });
  ...
  await expect(toast.locator('[role="alert"][aria-atomic="true"]')).toHaveCount(
    1,
  );
...
test("custom two-second toast pauses while hovered and resumes on leave", async ({
  page,
}) => {
  await page.goto("/components/toast/block#custom_duration");
  await page
    .locator("#dx-preview-block-root")
    .getByRole("button", { name: "2s — quick confirm", exact: true })
    .click();
  const toast = page
    .locator('[role="alertdialog"]:visible')
...
test("promise toasts transition from loading to success and error", async ({
  page,
}) => {
  await page.goto("/components/toast/block#loading");
  const block = page.locator("#dx-preview-block-root");
  await block
    .getByRole("button", { name: "Save (success)", exact: true })
    .click();
  await expect(
    page
      .locator('[role="alertdialog"]:visible')
      .filter({ hasText: "Saving your changes…" }),
  ).toBeVisible();
  await expect(
    page
      .locator('[role="alertdialog"]:visible')
  ...
  await block
    .getByRole("button", { name: "Publish (error)", exact: true })
    .click();
  await expect(
    page
      .locator('[role="alertdialog"]:visible')
      .filter({ hasText: "Publishing…" }),
  ).toBeVisible();
  await expect(
    page
      .locator('[role="alertdialog"]:visible')
      .filter({ hasText: "Publish failed" }),
  ).toBeVisible({ timeout: 3500 });
  await expect(
    page.getByRole("alertdialog", { name: "Publish failed" }),
  ).toBeVisible({ timeout: 3500 });
});
