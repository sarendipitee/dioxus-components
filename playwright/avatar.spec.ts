...
async function loadAvatarDemo(page: Page) {
  ...
  await page.route(ERROR_AVATAR_URL, (route) => route.abort("failed"));
...
test("renders a loaded avatar with its accessible image attributes", async ({ page }) => {
...
test("keeps a loading avatar image present and decorative", async ({ page }) => {
  await loadAvatarDemo(page);
