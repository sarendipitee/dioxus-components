...
test("toggle forwards global attributes", async ({ page }) => {
  ...
  await expect(button).toHaveAttribute(
    "data-audit",
    "toggle-global-attributes",
  );
});
test("controlled toggle synchronizes semantic and styling states and focuses on click", async ({
  page,
}) => {
  const root = await loadToggle(page);
  const button = toggle(root, "Controlled toggle");
  const offBackground = await button.evaluate(
...
test("controlled toggle activates once with Space and Enter", async ({
  page,
}) => {
  const root = await loadToggle(page);
  const button = toggle(root, "Controlled toggle");
...
test("controlled callback reports each requested state exactly once", async ({
  page,
}) => {
  const root = await loadToggle(page);
  const button = toggle(root, "Controlled toggle");
...
test("uncontrolled toggle honors default pressed and reports each transition once", async ({
  page,
}) => {
  const root = await loadToggle(page);
...
test("disabled toggles cannot receive focus or change state", async ({
  page,
}) => {
  const root = await loadToggle(page);
  ...
  await expect(root.locator("#controlled-count:visible").first()).toHaveText(
    "0",
  );
});
