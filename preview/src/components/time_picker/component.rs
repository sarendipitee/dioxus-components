...
use dioxus_primitives::{
    ...
    time_picker::{
        ...
        TimePickerAmPmSegmentProps, TimePickerFormat, TimePickerHourSegmentProps,
        TimePickerLabels, TimePickerMinuteSegmentProps, TimePickerSecondSegmentProps,
        TimePickerSeparatorProps, TimePickerSteps, TimePickerType, TimePickerValue,
    },
...
pub fn get_time_range(start: Time, end: Time, step_seconds: u32) -> Vec<TimePickerPreset> {
    time_picker_preset_range(start, end, step_seconds)
        .into_iter()
        .map(|value| TimePickerPreset::new(format_time(value, true, TimePickerFormat::TwentyFourHour), value))
        .collect()
}
...
pub fn TimePicker(props: TimePickerProps) -> Element {
    ...
    let show_right_section =
        props.right_section.is_some() && props.clear_section_mode != TimePickerClearSectionMode::Clear;
    ...
    rsx! {
        div {
            class: Styles::dx_time_picker_field,
            if let Some(label) = props.label.clone() {
                label { class: Styles::dx_time_picker_label, {label} }
            }
            if let Some(description) = props.description.clone() {
                div { class: Styles::dx_time_picker_description, {description} }
            }
                    ...
                    PopoverRoot {
                        ...
                        TimePickerInput {
                            ...
                            if show_right_section {
                                span { class: Styles::dx_time_picker_right_section, {props.right_section} }
                            }
            ...
            if let Some(error) = props.error.clone() {
                div { class: Styles::dx_time_picker_error, role: "alert", {error} }
            }
        }
    }
}
...
pub(crate) fn TimePickerInput(props: TimePickerInputProps) -> Element {
    let base = attributes!(div {
        class: Styles::dx_time_picker_group
    });
    let merged = merge_attributes(vec![base, props.attributes]);
...
pub(crate) fn TimePickerClearButton(props: TimePickerClearButtonProps) -> Element {
    rsx! {
        button {
            type: "button",
            class: Styles::dx_time_picker_clear,
...
pub(crate) fn TimePickerPopoverTrigger(props: PopoverTriggerProps) -> Element {
    rsx! {
        PopoverTrigger {
            class: Styles::dx_time_picker_popover_trigger,
...
pub(crate) fn TimePickerDropdown(props: TimePickerDropdownProps) -> Element {
    ...
    let select = move |time: Time| {
        props.on_picker_value_change.call(Some(TimePickerValue::Time(time)));
    ...
    rsx! {
        TimePickerPopoverContent {
            ...
            align: props.align,
            style,
            div { class: Styles::dx_time_picker_dropdown_grid,
            ...
            for group in props.preset_groups.clone() {
                div { class: Styles::dx_time_picker_preset_group,
                    div { class: Styles::dx_time_picker_preset_group_label, {group.label} }
                    TimePickerPresetList {
                        selected_time: props.selected_time,
                        selected_value: props.selected_value,
                        on_value_change: props.on_value_change,
...
pub(crate) fn TimePickerDropdownColumn(props: TimePickerDropdownColumnProps) -> Element {
    rsx! {
        div { class: Styles::dx_time_picker_dropdown_column,
            div { class: Styles::dx_time_picker_dropdown_label, {props.label} }
            div {
                class: Styles::dx_time_picker_dropdown_options,
...
pub(crate) fn TimePickerDropdownOption(props: TimePickerDropdownOptionProps) -> Element {
    rsx! {
        button {
            type: "button",
            class: Styles::dx_time_picker_dropdown_option,
...
pub(crate) fn TimePickerPresetList(props: TimePickerPresetListProps) -> Element {
    ...
    rsx! {
        div { class: Styles::dx_time_picker_preset_row,
            for preset in props.presets.clone() {
                button {
                    type: "button",
                    class: Styles::dx_time_picker_preset,
...
fn format_time(time: Time, with_seconds: bool, format: TimePickerFormat) -> String {
    match (format, with_seconds) {
        ...
        (TimePickerFormat::TwelveHour, false) => {
            ...
            format!("{:02}:{:02} {suffix}", display_hour(time.hour()), time.minute())
        }
